...
int NoAudioCodec::Write(const int16_t* data, int samples) {
    ...
    if (tx_buffer_.size() < (size_t)samples) {
    ...
    esp_err_t ret = i2s_channel_write(tx_handle_, tx_buffer_.data(), samples * sizeof(int32_t), &bytes_written, pdMS_TO_TICKS(200));
    if (ret != ESP_OK) {
...
int NoAudioCodec::Read(int16_t* dest, int samples) {
    size_t bytes_read;
    if (rx_buffer_.size() < (size_t)samples) {
        rx_buffer_.resize(samples);
    }
    if (i2s_channel_read(rx_handle_, rx_buffer_.data(), samples * sizeof(int32_t), &bytes_read, pdMS_TO_TICKS(200)) != ESP_OK) {
